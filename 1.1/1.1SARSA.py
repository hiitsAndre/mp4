...
def Q_learning():
    ...
    while count < game_num:
        ...
        current_action = select_action(current_state, Q, epsilon)
        bounce = 0
        while True:
            key = (discretize_state(current_state), current_action)
            # update Q,N
            next_state, current_reward = simulate_one_time(current_state, current_action)
            # consider state is none
            if next_state != None:
                next_action = select_action(next_state, Q, epsilon)
                next_state_Q = Q[(discretize_state(next_state), next_action)]
            else:
                next_state_Q = -1
            N[key] += 1
            alpha = C / (C + N[key])
            Q[key] = Q[key] + alpha * (current_reward + gamma * next_state_Q - Q[key])
    ...
    with open('Qtable_S.pkl', 'wb') as f:
        pickle.dump(Q, f)
    with open('Ntable_S.pkl', 'wb') as f:
        pickle.dump(N, f)
    ...
    while test_count < test_num:
        ...
        bounce = 0
        while True:
            selected_action = select_action(current_state, Q, epsilon_test)
